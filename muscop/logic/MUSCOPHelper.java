...
public class MUSCOPHelper extends ProtocolHelper {
	...
	public void initializeDataStructures() {
		...
		MUSCOPParam.indexMaster = new AtomicInteger[numUAVs];
		MUSCOPParam.alive = new AtomicBoolean[numUAVs];
		for (int i = 0; i < numUAVs; i++) {
			state[i] = new AtomicInteger();	// Implicit value State.START, as it is equals to 0
			MUSCOPParam.indexMaster[i] = new AtomicInteger();
			MUSCOPParam.alive[i] = new AtomicBoolean(true);
		}
